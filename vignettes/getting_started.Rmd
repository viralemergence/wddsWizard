...
Its composed two components: disease data and a project metadata.
We will go through each component in more detail later in the vignette. 
...
wddsWizard::data_required_fields
```
...
all(wddsWizard::data_required_fields %in% names(my_disease_data))
...
details on preparing those components. The general strategy is to get each component into a data frame, then use specific methods to ensure they are in the proper
