...
Creators are tricky because they have subcomponents. Lets make sure our creators
are properly formatted! 

We can use the `reference` argument in `json_validator` to only validate against
part of the schema. This is extremely helpful if you want to isolate validation
issues.

```{r validate-project}
schema <- here::here("inst/extdata/wdds_schema/wdds_schema.json")

creators_validator <- jsonvalidate::json_validator(schema,engine = "ajv",reference = "#/properties/creators")

creators_validation <- creators_validator(project_metadata_json)
